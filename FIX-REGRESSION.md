...
| 42 | 查看存储：没怎么用就 1GB（大头是「自动备份快照」=每次手动导出把全部数据 base64 放大复制一份，约 800MB） | personalize.js 查看存储页「自动备份快照」分类新增**删除按钮**（只删副本键 xy-home-v2:__auto-backup-snapshot，LS+IDB 都删，真实数据全保留；openModal 二次确认→确定后禁按钮+toast，下次导出自动重建）；存储页顶部「总占用」下加警示行 st-snap-hint（仅快照存在时显示）+ 分类表该行高亮 .snap 点破"它是完整副本、删了不伤真实数据" | 哨兵 `st-clear-snap` / 验证：设置→查看存储→顶部警示+分类高亮→点删除→确定后快照归零、配额降约 800MB、聊天/头像/音乐无损 |
...
| 49 | 吃饭提醒的夜宵时间提醒没有对应字卡，很突兀 | 根因：饭点提醒 4 个窗口（早/午/晚/夜宵 21:30–23:30）共用「提醒吃饭/追问关心」通用话术池（DEFAULT_CARD_DATA.eat 两组），夜宵触发抽到的还是「到饭点啦」「饭要按时吃」等"按时吃饭"语境文案，深夜发出违和。修复：① default-cards-data.js 的 `DEFAULT_CARD_DATA.eat` 新增「夜宵提醒」（10 条）/「夜宵关心」（5 条）两组，字卡库【吃什么】tab 自动显示并可逐张开关（dc-off-eat:* 联动）；② p2-features.js `eatRemindFire` 按 `code==='nightcap'` 分别抽「夜宵提醒/夜宵关心」池（DEF_EAT_REMIND_NIGHT / DEF_EAT_REMIND_NIGHT_CARE 兜底同源），其余窗口仍走通用池；夜宵话术刻意不用 `{d}` 占位，避免空菜单替换成「饭」的违和 | 哨兵 `DEF_EAT_REMIND_NIGHT` / 验证：调系统时间到 21:30–23:30 或临时把 eatRemindProb 设 100 → 夜宵窗口触发后聊天收到的是夜宵语境话术（如「夜深了，饿不饿？想吃点夜宵吗」）而非「到饭点啦」；字卡库【吃什么】tab 出现「夜宵提醒/夜宵关心」分组，关掉其中一张后夜宵触发不再出现该条 |
