...
const chrome = spawn(chromePath, [
  ...
  '--user-data-dir=' + join(process.env.TEMP || '/tmp', 'mochi-gc-scene-' + Date.now()),
...
// ===== 场景 A：开启群聊后，在设置页直接刷新 =====
console.log('===== 场景 A：设置页直接刷新 =====');
await gotoHome();
await openChatSettings();
await enableGroupChat();
await checkState('A.开启后(设置页):');
await gotoHome();
await checkState('A.刷新后(首页):');

// ===== 场景 B：开启群聊 → 进装修模式 → 退出 → 检查 =====
console.log('\n===== 场景 B：装修模式退出后 =====');
await gotoHome();
await openChatSettings();
await enableGroupChat();
...
await sleep(500);
await checkState('B.开启后(首页):');
// 进装修模式
await evalJs("(function(){var b=document.getElementById('desk-decor-btn');if(b)b.click();return true;})()");
await sleep(700);
const inDecor = await evalJs("(function(){var g=document.querySelector('.app-grid.editing');return !!g;})()");
console.log('B.进入装修模式:', inDecor);
// 退出装修模式
await evalJs("(function(){var b=document.getElementById('desk-decor-exit');if(b)b.click();return true;})()");
await sleep(700);
await checkState('B.退出装修后:');

// ===== 场景 C：开启群聊 → 切到桌面 B → 切回 → 检查 =====
console.log('\n===== 场景 C：切换桌面再切回 =====');
await gotoHome();
await openChatSettings();
await enableGroupChat();
...
await evalJs("(function(){var b=document.getElementById('chat-back');if(b)b.click();return true;})()");
await sleep(500);
await checkState('C.开启后(首页):');
// 创建并切到新桌面
const cidB = await evalJs("window.createContact('三宝')");
await evalJs("window.setActiveContact('" + cidB + "')");
await sleep(600);
await checkState('C.切到桌面B(B未开群聊):');
// 切回原桌面
await evalJs("window.setActiveContact('default')");
await sleep(600);
await checkState('C.切回原桌面:');

// ===== 场景 D：开启群聊 → 切到桌面 B → 在 B 刷新 → 切回 =====
console.log('\n===== 场景 D：切到B后刷新再切回 =====');
await gotoHome();
await openChatSettings();
await enableGroupChat();
...
await evalJs("(function(){var b=document.getElementById('chat-back');if(b)b.click();return true;})()");
await sleep(500);
await checkState('D.开启后(首页):');
const cidD = await evalJs("window.createContact('四宝')");
await evalJs("window.setActiveContact('" + cidD + "')");
await sleep(600);
await checkState('D.切到桌面D:');
await gotoHome();
await checkState('D.刷新后(在D桌面):');
await evalJs("window.setActiveContact('default')");
await sleep(600);
await checkState('D.切回原桌面:');
