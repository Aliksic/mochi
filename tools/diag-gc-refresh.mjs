// ===== 诊断：群聊刷新后是否保持开启 =====
// 复现：开启群聊 → 检查 localStorage → 刷新 → 检查群聊按钮/开关状态
...
const chrome = spawn(chromePath, [
  ...
  '--user-data-dir=' + join(process.env.TEMP || '/tmp', 'mochi-gc-ref-' + Date.now()),
...
console.log('active-contact:', await evalJs("localStorage.getItem('xy-home-v2:active-contact')"));
console.log('__activeCid:', await evalJs("window.__activeCid"));
console.log('activePrefix:', await evalJs("window.activePrefix()"));
console.log('store.get(group-chat-enabled):', await evalJs("window.activeStore().get('group-chat-enabled')"));

// 检查群聊按钮状态
const d1 = JSON.parse(await evalJs("(function(){var gc=document.querySelector('.app[data-app=\"group-chat\"]');var pool=document.getElementById('desk-widget-pool');var mainGrid=document.querySelector('.app-grid[data-app=\"main\"]');return JSON.stringify({gcHidden:gc?gc.hidden:'n/a',gcInPool:gc?gc.parentNode===pool:'n/a',gcInMain:gc?gc.parentNode===mainGrid:'n/a'});})()") || '{}');
...
console.log('刷新后 activePrefix:', await evalJs("window.activePrefix()"));
