...
try {
  ...
  const tab = await evalJs("(function(){ var t = document.querySelector('#dc-tabs [data-type=\"fish\"]'); if (!t) return { tab: false }; var pg = document.getElementById('page-default-cards'); pg.hidden = false; document.getElementById('dc-tabs').querySelector('[data-type=\"fish\"]').click(); var headers = Array.prototype.map.call(document.querySelectorAll('#dc-list .ccg-name'), function (x) { return x.textContent; }); var cards = document.querySelectorAll('#dc-list .cc-item').length; return { tab: true, headers: headers, cards: cards }; })()");
  ...
  await evalJs("document.getElementById('page-default-cards').hidden = true; true");

  console.log('\n== T8 花园/同频/伸手/喝水/存钱罐 预设池 ==');
  const pools = await evalJs("({ tabs: ['garden','sync','reach','water','piggy'].map(function(k){ return !!document.querySelector('#dc-tabs [data-type=\"'+k+'\"]'); }).every(Boolean), g: window.getLibPool('garden','梦角悄悄话',[]).length, s: window.getLibPool('sync','TA 此刻',[]).length, r: window.getLibPool('reach','悄悄话',[]).length, w: window.getLibPool('water','提醒模板',[]).length, wt: window.getLibPool('water','TA 提醒句式',[]).length, p: window.getLibPool('piggy','存入碎碎念',[]).length })");
  ...
  const waterTab = await evalJs("(function(){ var pg = document.getElementById('page-default-cards'); pg.hidden = false; document.querySelector('#dc-tabs [data-type=\"water\"]').click(); var headers = Array.prototype.map.call(document.querySelectorAll('#dc-list .ccg-name'), function (x) { return x.textContent; }); var cards = document.querySelectorAll('#dc-list .cc-item').length; pg.hidden = true; return { h: headers.join(','), c: cards }; })()");
  ok('喝水 tab 渲染 5 组 21 张', waterTab && waterTab.h === '提醒模板,TA 提醒句式,他视角温柔提醒,喝够夸奖,继续鼓励' && waterTab.c === 21, waterTab);
