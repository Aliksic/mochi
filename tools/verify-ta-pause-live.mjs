...
(async () => {
  try {
    ...
    if (playing && ((playing.hasAudio && playing.paused === false) || playing.musicPlaying)) {
      ...
      let pauseCardAt = -1, resumeCardAt = -1;
      const t0 = Date.now();
      while (Date.now() - t0 < 35000) {
        const r = await evalJs(`(function(){
          ...
          const all = txts.join('|');
          const hasPause = /先暂停一下|让音乐停一会儿|先搁一搁|我有话想跟你说|陪我一下下|按下了暂停键/.test(all);
          const hasResume = /继续听吧|按了播放|等急了|音乐继续|按下了播放键|说完啦/.test(all);
          return { count: txts.length, hasPause, hasResume, all: all.slice(0, 300) };
        })()`);
        if (pauseCardAt < 0 && r.hasPause) pauseCardAt = Date.now() - t0;
        if (r.hasResume) { resumeCardAt = Date.now() - t0; break; }
        await sleep(500);
      }
