...
const FIX_SENTINELS = [
  ...
  { name: '#144 armFgIdbReset 补 touchMac 分支（伪装 UA 的 iPad 回前台重建 IDB 连接）', file: 'js/idb.js', needle: 'if (!/iPhone|iPad|iPod/i.test(ua) && !touchMac) return;' },
  ...
  { name: '#148 fs 态写 --mochi-ios-h（#179 公式：envTop+inner，覆盖=整屏/已避让=inner）', file: 'js/mobile-adapt.js', needle: 'Math.min(_safeTop + _ih2, _sh2' },
  ...
  { name: '#150+#161 后台来电通知辅助（bgCallNotify：SW 链路弹「XX来电」，force+avFixed；#161 加 hint 尾缀）', file: 'js/call.js', needle: 'function bgCallNotify(name, hint) {' },
  { name: '#161 响铃挂起写入（holdIncomingCall：后台来电存 call-hold 全局根键+发可接听通知，不再即判未接）', file: 'js/call.js', needle: 'function holdIncomingCall(name, cid) {' },
  ...
  { name: '#199 浏览器覆盖形态·env 探针扩展（雨见/Via 沉浸式安卓壳 screen==inner 非 standalone：原只认 ios-pwa-standalone，35px 系统栏无人避让=模拟状态栏钻顶 #114 形态；改回只认 standalone 此分支即消失）', file: 'js/mobile-adapt.js', needle: "var _coverBrowser = !d.classList.contains('ios-pwa-standalone')" },
  ...
  { name: '#199 判定器·浏览器覆盖形态期望底边=可视区底（.phone 刻意不超 inner，仍按 envTop+inner 判则修好后误报 #179 少填）', file: 'js/device.js', needle: 'const coverBrowser = !inp.standalone && diff <= 2 && envTop >= 20;' },
  ...
  { name: '#203 判定器·保留形态期望底边=inner（.phone 超 inner=文档滚动量，仍按 envTop+inner 判则修好后误报）', file: 'js/device.js', needle: 'const expBase = (coverBrowser || resStand) ? inp.innerH : (inp.envTop || 0) + inp.innerH;' },
