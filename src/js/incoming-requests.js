...
// 归属：AI-A（业务功能）。依赖 idb.js/contacts.js/personalize.js(openModal)/chat.js。
(function () {
  ...
  // 设置页开关行（动态插入第一个 set-group，紧跟「开启群聊」行；样式复用 .set-row/.toggle）
  // 全桌面通：根键不随联系人隔离，切桌面/回填后只需同步一次勾选态。
  (function () {
    try {
      if (document.getElementById('sf-desk-checkin-row')) return;
      const anchor = document.getElementById('sf-group-chat-row');
      if (!anchor) return;
      const row = document.createElement('div');
      row.className = 'set-row';
      row.id = 'sf-desk-checkin-row';
      row.innerHTML =
        '<div class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="#111111" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2v3M4.2 4.2l2.2 2.2M2 12h3M19 12h3M4.2 19.8l2.2-2.2M17.6 17.6l2.2 2.2"/><path d="M12 6a6 6 0 016 6v4h-3v-4a3 3 0 00-6 0v4H6v-4a6 6 0 016-6z"/><path d="M9 20h6"/></svg></div>' +
        '<div class="txt">桌面查岗</div>' +
        '<div class="val">其他桌面的TA来查岗</div>' +
        '<label class="toggle"><input type="checkbox" id="sf-desk-checkin"><span class="tk"></span></label>';
      anchor.parentNode.insertBefore(row, anchor.nextSibling);
      const input = row.querySelector('input');
      const sync = function () { const v = deskCheckinEn(); if (v !== input.checked) input.checked = v; };
      sync();
      input.addEventListener('change', function () {
        if (input.checked === deskCheckinEn()) return;
        window.setDeskCheckinEn(input.checked);
        if (typeof window.toast === 'function') {
          window.toast(input.checked ? '桌面查岗已开启：其他桌面的TA会来查岗、找你聊天' : '桌面查岗已关闭：其他桌面的TA不再来打扰（聊天记录不受影响）');
        }
      });
  ...
  function deliver(req) {
    ...
    const title = req.kind === 'chat' ? name + ' 想找你聊天' : name + ' 来查岗了';
    if (document.hidden) {
      if (window.bgNotifyCheck) window.bgNotifyCheck(title + '：' + req.text, Date.now(), { name: name + (req.kind === 'chat' ? '来聊天' : '查岗') });
      return true;
    }
    if (!window.openModal) return true;
    const okText = req.kind === 'chat' ? '同意' : '现在回TA';
    const lateOkText = '现在回TA';
    window.openModal(title, '', function (v) {
      ...
      noInput: true,
      lock: true,
      staticText: (req.kind === 'chat' ? '想和你聊聊天，忙完记得过来。' : '想看看你在做什么，来陪陪我呀。') + '\n' + (req.text || ''),
      pills: [{ label: '稍后', value: 'later' }, { label: okText, value: 'reply' }]
    });
    return true;
  }

  // 切换 + 进聊天 + 等加载就绪后 TA 当场发话
  ...
  // 调度：遍历所有联系人，非激活桌面按各自配置掷概率
  function maybeIncoming() {
    try {
      if (!deskCheckinEn()) return; // 全局开关关闭：不触发任何跨桌面查岗/求聊天
      ...
      list.forEach(function (c) {
        ...
        if (num(cfg, 'ckq-en', 0) === 1) {
          ...
          if (Date.now() - lastAt(cid, 'checkin') >= cool * 60000 && Math.random() * 100 < prob) {
            ...
            if (q && q.text) {
              deliver({ cid: cid, kind: 'checkin', text: q.text, q: q, ts: Date.now(), status: 'pending' });
              return;
            }
          }
        }
        // 求聊天：开关 + 概率 + 冷却（as-*）
        if (num(cfg, 'as-en', 0) === 1) {
  ...
  window.triggerIncomingCheckin = function (cid) {
    if (!deskCheckinEn()) { try { if (window.toast) window.toast('桌面查岗已关闭（可在设置里开启）'); } catch (e) {} return false; }
